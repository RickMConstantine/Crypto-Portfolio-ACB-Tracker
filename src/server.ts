...
async function validateTransaction(transaction: Transaction | Omit<Transaction, 'id'>) {
  ...
  // Wallet validation
  if (rules.requiresFromWallet && !transaction.from_wallet_name) {
    throw new Error(`From Wallet is required for ${transaction.type}.`);
  }
  if (rules.forbidsFromWallet && transaction.from_wallet_name) {
    throw new Error(`From Wallet is not allowed for ${transaction.type}.`);
  }
  if (rules.requiresToWallet && !transaction.to_wallet_name) {
    throw new Error(`To Wallet is required for ${transaction.type}.`);
  }
  if (rules.forbidsToWallet && transaction.to_wallet_name) {
